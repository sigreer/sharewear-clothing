...
cd "$PROJECT_ROOT/apps/agent-observer/apps/server"
bun run start &
SERVER_PID=$!
sleep 3
...
cd "$PROJECT_ROOT/apps/agent-observer/apps/demo-cc-agent"
...
echo "1. In terminal 1: cd apps/agent-observer/apps/server && bun run dev"
echo "2. In terminal 2: cd apps/agent-observer/apps/client && bun run dev"
