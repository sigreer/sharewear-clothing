...
FABRIC_COLOR=""
BACKGROUND_COLOR=""
TEMPLATE="assets/t-shirt-template.png"
BLEND_FILE="assets/original-blender-template.blend"
...
if [ -n "$COMPOSE_PRESET" ]; then
    ...
    COMPOSE_CMD=(python3 compose_design.py --template "$TEMPLATE" --design "$DESIGN" --preset "$COMPOSE_PRESET" --output "$COMPOSITED")
...
BLENDER_CMD=(blender --background "$BLEND_FILE" --python render_design.py -- "$BLEND_FILE" "$RENDER_INPUT" "$OUTPUT_DIR" "$SAMPLES")
